package com.ankamagames.dofus.logic.game.approach.frames
{
   ...
   public class GameServerApproachFrame implements Frame
   {
      ...
      public function process(msg:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case msg is CharactersListMessage:
               ...
               {
                  misc = Kernel.getWorker().getFrame(MiscFrame) as MiscFrame;
                  feature = OptionalFeature.getOptionalFeatureByKeyword("character.xp.bonusForYoungerCharacters");
                  bonusXpFeatureActivated = !feature || misc.isOptionalFeatureActive(feature.id);
               ...
               if(Boolean(this._charactersList.length) && !isCharacterCreationDisplayForced)
               {
            ...
            case msg is CharacterAutoConnectAction:
               ...
               if(Boolean(characterToConnect) && Boolean(currentServer))
               {
            ...
            case msg is AllModulesLoadedMessage:
               ...
               this._gmaf = null;
               DofusErrorHandler.debugManuallyActivated = BuildInfos.BUILD_TYPE != BuildTypeEnum.RELEASE;
               LogLogger.activeLog(BuildInfos.BUILD_TYPE != BuildTypeEnum.RELEASE);
               ...
               if(Boolean(InterClientManager.getInstance().flashKey) && (!PlayerManager.getInstance() || PlayerManager.getInstance().server.id != 129 && PlayerManager.getInstance().server.id != 130))
               {
      ...
      private function getCharacterColorsInformations(ctrci:*) : Vector.<int>
      {
         ...
         var num:int = int(ctrci.colors.length);
         for(var i:int = 0; i < num; )
         {
            uIndexedColor = Number(ctrci.colors[i]);
      ...
      private function requestCharactersList() : void
      {
         ...
         if(Boolean(ConnectionsHandler) && Boolean(ConnectionsHandler.getConnection()))
         {
      ...
      private function launchAutoConnect(charToConnect:BasicCharacterWrapper, server:Server) : void
      {
         ...
         if(Boolean(charToConnect) && ((server.gameTypeId != GameServerTypeEnum.SERVER_TYPE_HARDCORE && server.gameTypeId != GameServerTypeEnum.SERVER_TYPE_EPIC || charToConnect.deathState == 0) && !SecureModeManager.getInstance().active && !this.isCharacterWaitingForChange(charToConnect.id) && !PlayerManager.getInstance().wasAlreadyConnected))
         {
            this._openCharsList = false;
            this._kernel.processCallback(HookList.CharactersListUpdated,this._charactersList);
